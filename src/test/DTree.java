...
public class DTree {
	...
	/**
	 * Instead of checking each index we'll skip every INDEX_SKIP indices unless
	 * there's less than MIN_SIZE_TO_CHECK_EACH
	 */
	...
	/**
	 * If the number of data points is less than MIN_NODE_SIZE, we won't
	 * continue splitting, we'll take the majority vote
	 */
	private static final int MIN_NODE_SIZE = 5;
	/** the number of data records */
	private int N;
	/**
	 * the number of samples left out of the boostrap of all N to test error
	 * rate
	 * 
	 * @see <a href=
	...
	public DTree(ArrayList<int[]> data, RandomForest forest, int num) {
		...
		ArrayList<int[]> train = new ArrayList<int[]>(N); // data becomes the
															// "bootstrap" -
															// that's all it
															// knows
		ArrayList<int[]> test = new ArrayList<int[]>();

		BootStrapSample(data, train, test, num);// populates train and test
												// using data
		testN = test.size();
		correct = 0;
	...
	public int realtime_CalculateClass(int[] test, int nu) {
		int kls = Evaluate(test);
		return kls;
	}

	public ArrayList<Integer> CalculateClasses(ArrayList<int[]> test, int nu) {
		ArrayList<Integer> corest = new ArrayList<Integer>();
		int k = 0;
		int korect = 0;
		for (int[] record : test) {
	...
	/**
	 * This creates the decision tree according to the specifications of random
	 * forest trees.
	 * 
	 * @see <a href=
	 *      "http://www.stat.berkeley.edu/~breiman/RandomForests/cc_home.htm#overview">Overview
	 *      of random forest decision trees</a>
	 * @param train
	 *            the training data matrix (a bootstrap sample of the original
	 *            data)
	 * @return the TreeNode object that stores information about the parent node
	 *         of the created tree
	 */
	 ...
	 * 
	 * <ul>
	 * <li>Step A Check if this node is a leaf, if so, it will mark isLeaf true
	 * and mark Class with the leaf's class. The function will not recurse past
	 * this point.</li>
	 * <li>Step B Create a left and right node and keep their references in this
	 * node's left and right fields. For debugging purposes, the generation
	 * number is also recorded. The {@link RandomForest#Ms Ms} attributes are
	 * now chosen by the {@link #GetVarsToInclude() GetVarsToInclude} function
	 * </li>
	 * <li>Step C For all Ms variables, first {@link #SortAtAttribute(List,int)
	 * sort} the data records by that attribute , then look through the values
	 * from lowest to highest. If value i is not equal to value i+1, record i in
	 * the list of "indicesToCheck." This speeds up the splitting. If the number
	 * of indices in indicesToCheck > MIN_SIZE_TO_CHECK_EACH then we will only
	 * {@link #CheckPosition(int, int, int, ImageAnalysisStatistics.DTree.DoubleWrap, ImageAnalysisStatistics.DTree.TreeNode)
	 * check} the entropy at every {@link #INDEX_SKIP INDEX_SKIP} index
	 * otherwise, we
	 * {@link #CheckPosition(int, int, int, ImageAnalysisStatistics.DTree.DoubleWrap, ImageAnalysisStatistics.DTree.TreeNode)
	 * check} the entropy for all. The "E" variable records the entropy and we
	 * are trying to find the minimum in which to split on</li>
	 * <li>Step D The newly generated left and right nodes are now checked: If
	 * the node has only one record, we mark it as a leaf and set its class
	 * equal to the class of the record. If it has less than
	 * {@link #MIN_NODE_SIZE MIN_NODE_SIZE} records, then we mark it as a leaf
	 * and set its class equal to the {@link #GetMajorityClass(List) majority
	 * class}. If it has more, then we do a manual check on its data records and
	 * if all have the same class, then it is marked as a leaf. If not, then we
	 * run {@link #RecursiveSplit(ImageAnalysisStatistics.DTree.TreeNode)
	...
	private void RecursiveSplit(TreeNode parent, int Ntreenum) {
		...
		if (!parent.isLeaf) {
			...
			for (int m : vars) {
				...
				SortAtAttribute(parent.data, m);// sorts on a particular column
												// in the row
				...
				if (indicesToCheck.size() == 0) {// if all the Y-values are
													// same, then get the class
													// directly
					parent.isLeaf = true;
				...
				if (indicesToCheck.size() > MIN_SIZE_TO_CHECK_EACH) {
					for (int i = 0; i < indicesToCheck.size(); i += INDEX_SKIP) {
						...
						if (lowestE.d == 0)// lowestE now has the minimum
											// conditional entropy so IG is max
											// there
							break;
					}
				} else {
	...
	/**
	 * Checks the {@link #CalcEntropy(double[]) entropy} of an index in a data
	 * matrix at a particular attribute (m) and returns the entropy. If the
	 * entropy is lower than the minimum to date (lowestE), it is set to the
	 * minimum.
	 * 
	 * The total entropy is calculated by getting the sub-entropy for below the
	 * split point and after the split point. The sub-entropy is calculated by
	 * first getting the {@link #GetClassProbs(List) proportion} of each of the
	 * classes in this sub-data matrix. Then the entropy is
	 * {@link #CalcEntropy(double[]) calculated}. The lower sub-entropy and
	 * upper sub-entropy are then weight averaged to obtain the total entropy.
	 * 
	 * @param m
	 ...
	 *            the index to check
	 * @param Nsub
	 *            the number of records in the data matrix
	 * @param lowestE
	...
	private double CheckPosition(int m, int n, int Nsub, DoubleWrap lowestE, TreeNode parent, int nTre) {
		...
		// System.out.println("Checking position of the index attribute of tree
		// :"+nTre);
		...
		// System.out.println("g:"+parent.generation+" N:"+parent.data.size()+"
		// M:"+RandomForest.M+" Ms:"+RandomForest.Ms+" n:"+n+" m:"+m+"
		// val:"+parent.data.get(n)[m]+" e:"+e);
	 ...
	 * @param nSplit
	 *            return all data records above this index in a sub-data matrix
	 ...
	 *            the data matrix to be split
	 * @param nSplit
	 *            return all data records below this index in a sub-data matrix
	...
	/**
	 * This class compares two data records by numerically comparing a specified
	 * attribute
	 * 
	 * @author kapelner
	 *
	...
	private class AttributeComparator implements Comparator {
		 ...
		 * 
		 * @param m
		 *            the attribute in which to compare on
		 */
	...
	/**
	 * Given a data matrix, return a probabilty mass function representing the
	 * frequencies of a class in the matrix (the y values)
	 * 
	 * @param records
	...
	/**
	 * Given a probability mass function indicating the frequencies of class
	 * representation, calculate an "entropy" value using the method in Tan
	 * Steinbach Kumar's "Data Mining" textbook
	 * 
	 * @param ps
	 ...
	 * @param test
	 *            the records that are absent in the bootstrap sample
	 */
	private void BootStrapSample(ArrayList<int[]> data, ArrayList<int[]> train, ArrayList<int[]> test, int numb) {
		...
		for (int i = 0; i < N; i++)
			if (!in.get(i))
				test.add((data.get(i)).clone());// System.out.println("created
												// testing-data for tree :
												// "+numb);//everywhere its set
												// to false we get those to test
												// data

	}

	/**
	 * Recursively deletes all data records from the tree. This is run after the
	 * tree has been computed and can stand alone to classify incoming data.
	 * 
	 * @param node
	...
	/**
	 * Get the number of data records in the test data matrix that were
